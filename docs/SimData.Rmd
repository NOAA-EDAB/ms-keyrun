...
*Consider a function to do this using species max size, the vonB k and the desired number of bins for more consistency and to easily change Nsizebins.* Do we need t0 as well? Haven't saved it.
...
binwidths <- function(Lmin, Lmax, vonBk, Nsizebins){
  ...
  bindef <- (Lmax-Lmin)/Nsizebins
  
  #minmaxvonBK$BTS_fall_allbox_effic1 %>% group_by(species) %>% mutate(bindef = (maxlen)/5)
  
  #vonBk tells you how many fine bins for fast growth
  # threshold cumprod of 0.05 to define last bin? tiny growth increment
  ...
  thresh <- 0.05
  which(cumprod(rep(1, Lmax)*exp(-vonBk)) > thresh) #gives 1 cm increments of fast growth?
