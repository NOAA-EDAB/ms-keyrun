...
# Achieved convergence tolerance: 8.316e-06 
    ...
    nls(length~vb1(avgage, Linf, K, t0), data = saithe1,
...
# Achieved convergence tolerance: 5.511e-06
 ...
 svvbgf2 <- svlenage %>%   
    dplyr::group_by(species) %>%
    group_modify(~broom::tidy(possibly(nls(length~vb1(avgage, Linf, K, t0), data = .x,
                  start = vbStarts(length~avgage,data = .x)), otherwise = NA_real_)))
 svvbgf3 <- svlenage %>%   
