...
for(s in names(len_comp_data)){
  ...
  growthpars <- left_join(svexp3, vbout.2) %>%
    mutate(growthType = 4) # use only vonB for now, can change later
  ...
  growthspp <- growthpars %>%
    ...
    ungroup() %>%
    select(-era) %>%
    remove_rownames %>% column_to_rownames(var="par") %>%
    write.csv(paste0(o.name,"growth_species_NOBA_era",maxera,"_",s,".csv"))
}
