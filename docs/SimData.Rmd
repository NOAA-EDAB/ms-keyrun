...
Now write input files in Gavin's format to have surveys together in one file:
...
Survey and catch datasets in Gavin's suggested format (all surveys together, all fleets together):
...
for(s in names(survObsBiom)){
  ...
  svbio <- survObsBiom[[s]][[1]] %>%
    ...
    dplyr::mutate(survey = s,
                  CV = 0.2) %>% #placeholder, can get actual value used to generate from config files
