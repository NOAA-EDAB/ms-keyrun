...
coast <- sf::st_read("gis/NES_LME_coast.shp", quiet = T) %>%
  sf::st_transform(.,crs=crs)
GB_strata <- sf::st_read("gis/GB_SOE_strata.shp",quiet=T) %>%
    sf::st_transform(.,crs=crs)
...
coast <- sf::st_read("gis/NES_LME_coast.shp", quiet = T) %>%
  sf::st_transform(.,crs=crs)
GB_strata <- sf::st_read("gis/GB_SOE_strata_stat.shp",quiet=T) %>%
    sf::st_transform(.,crs=crs)
...
```{r calc_props_st, eval=T,echo = F, fig.align='center',fig.cap="Proportion of landings in GB relative to statistical area"}
...
```{r calc_lands,  eval=T, echo = F,fig.align='center',fig.cap="Total landings by statistical area"}
