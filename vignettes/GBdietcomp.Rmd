...
We visualize diets below. Hover over a bar to see which prey species is represented. In this visualization, color coding is only consisten within, not across, predators.
...
preycolcode <- data.frame(prey=names(plist2),
                          preycol=c(preycol)) %>%
  filter(!is.na(prey))
```
...
plist3 = lapply(split(tmp, tmp$Name), function(d) {
  #p1 <- 
   ggplot(d, aes(year, value, fill=prey)) +
   geom_bar(stat = "identity") + #
   ylab("Percent in Diet") +
