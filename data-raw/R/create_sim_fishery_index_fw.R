...
create_sim_fishery_index_fw <- function(atlmod,fitstart=NULL,fitend=NULL,saveToData=T) {
  ...
  if(any(is.na(allcatch$Name))){
    spp <- atlantisom::load_fgs(d.name, "nordic_groups_v04.csv") %>% 
      dplyr::select(Code, Name1 = Name)
    
    fillspp <- allcatch %>%
      dplyr::left_join(spp) %>%
      dplyr::mutate(Name = coalesce(Name, Name1))
    
    allcatch <- dplyr::select(fillspp, -Name1)
  }

  simCatchIndexSubannualFW <- allcatch
  
  #build new annual index from this one
  simCatchIndexFW <- allcatch %>%
  ...
  if (saveToData) {
    usethis::use_data(simCatchIndexSubannualFW, overwrite = TRUE)
    usethis::use_data(simCatchIndexFW, overwrite = TRUE)
  }
  
  return(simCatchIndexSubannualFW)
